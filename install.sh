...
# -----
# Last Modified: Sunday, 24th January 2021 2:47:15 pm
...
function install_scrutiny {
    ...
    install -m 644 ${PROJECT_PATH}/source/scrutiny.yaml /opt/scrutiny/config/scrutiny.yaml
    _update_stage_header ${?}

    # Install systemd unit
    _stage_header "Installing 'scrutiny.service' unit"
    install -m 644 ${PROJECT_PATH}/source/scrutiny.service /etc/systemd/system/scrutiny.service &>> ${SCRIPT_LOG_FILE}
    _update_stage_header ${?}
    _stage_header "Reloading systemd units"
    systemctl daemon-reload &>> ${SCRIPT_LOG_FILE}
    _update_stage_header ${?}
    ...
    install -m 644 ${PROJECT_PATH}/source/scrutiny-collector-cron /etc/cron.d/scrutiny-collector-cron
    _update_stage_header ${?}
